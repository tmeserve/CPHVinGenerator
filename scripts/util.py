...
def calculateCheckDigit(vin):
    ...
    if sum([x[0] * x[1] for x in zip([transliterationString.find(c) % 10 for c in vin], valWeights)]) % 11 == cd:
        return [(True, vin)]
    else:
        true_list = []
        for i in range(9):
            y = i + 1
            the_sum = sum([x[0] + y * x[1] for x in zip([transliterationString.find(c) % 10 for c in vin], valWeights)]) % 11
            if the_sum == cd:
                true_list.append((True, vin, 'Valid VIN'))
        return true_list
...
def ValidateVIN(vin, many=False):
    if many:
        ...
        calculated = calculateCheckDigit(vin)
        if not calculated:
            return (False, vin, 'Unable to Generate')
        
        return random.choice(calculated)
